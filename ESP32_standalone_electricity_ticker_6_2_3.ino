/*
  ESP32_standalone_electricity_ticker_6_2_3.ino
  -----------------------------------------------------
...
*/
 ...
 // ========================================================================
 // Dynamic Electricity Ticker for XIAO ESP32C3 - VERSION 6.2.3
...
// For example: at 17:57, the current interval is 17:45-18:00, so we return index for 17:45.
// This is different from finding the "next" interval (which would be 18:00 at 17:57).
// Returns index into prices/unixSeconds arrays, or -1 if not found.
int findCurrentPriceIndex(const JsonArray& unixSeconds) {
    ...
    // We want to find the LAST entry where unixSeconds < nextQuarterHourBoundary
    // This gives us the interval we're currently IN (not the next one)
    ...
    // (This entry is the current interval we're in)
    ...
    // Edge case: current time is before first entry (shouldn't happen with API data)
    return 0;
}
...
void displaySecondaryList() {
    ...
    snprintf(lines[18], sizeof(lines[18]), "price ticker v6.2.3");
...
void setup() {
    Serial.begin(115200);
    debugPrint(2, "Starting Dynamic Electricity Ticker v6.2.3 (15-MINUTE DETAIL MODE) - DST-SAFE");
