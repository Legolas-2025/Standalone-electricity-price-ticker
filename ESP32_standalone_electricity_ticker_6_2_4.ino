/*
  ESP32_standalone_electricity_ticker_6_2_4.ino
  -----------------------------------------------------

  VERSION 6.2.4 CHANGES (2026-04-01):
  ----------------------------------
  FIX: Exact-boundary display refresh bug
  - Problem: At the exact top of the hour (e.g., 20:00:00), the display 
    automatically refreshed but showed the PREVIOUS hour's data (19:00). 
    This happened because the "next-boundary" rounding logic in 
    findCurrentPriceIndex() incorrectly excluded the current interval if 
    the time was exactly on the boundary.
  - Fix: Simplified findCurrentPriceIndex() to use a robust "last entry <= now" 
    comparison. This ensures the display transitions to the new hour 
    instantaneously at 00:00:00.

VERSION 6.2.3 CHANGES (2026-04-01):
  ----------------------------------
  ...
  VERSION 6.2.2 CHANGES (2026-03-31):
  ----------------------------------

  FIX: Display blank lines issue
  ...
  VERSION 6.2.1 CHANGES (2026-03-30):
  ----------------------------------

  BUG FIX: findCurrentPriceIndex() now returns correct current interval
  - Problem: The function was finding the NEXT 15-minute interval (first >= now)
    instead of the CURRENT interval (the one we're IN).
  - At 17:57, it returned index for 18:00 instead of 17:45
  - Result: Display showed hour 18 instead of hour 17
  - Fix: Now calculates next 15-minute boundary and finds the last entry < that boundary

  VERSION 6.2.0 CHANGES (2026-03-29):
  ----------------------------------
  DST FIX:
  - CRITICAL: Fixed DST (Daylight Saving Time) handling on switch days.
  - Previously, the code assumed every day has exactly 24 hours × 4 quarters = 96 price entries.
    This caused incorrect display on DST days:
    - Spring forward (March): Only 23 hours exist → 92 entries
    - Fall back (October): 25 hours exist → 100 entries
  - FIX: All price lookups now use timestamp-based searching through unix_seconds array
    instead of arithmetic calculation (hourIndex * 4).
  - New functions added: findPriceIndexForHour(), findCurrentPriceIndex(), getHourFromPriceIndex()
  - Updated functions: getHourlyAverage(), display15MinuteDetails(), displayPriceRow(),
    displayPrimaryList(), updateLeds()
  - This makes the ticker work correctly on all days, including DST switch days.
...
*/
 ...
 // ========================================================================
 // Dynamic Electricity Ticker for XIAO ESP32C3 - VERSION 6.2.4
 // 15-MINUTE DETAIL MODE, DST-SAFE via timestamp-based lookups (CET <-> CEST automatic)
 // ========================================================================
...
void displaySecondaryList() {
    ...
    snprintf(lines[18], sizeof(lines[18]), "price ticker v6.2.4");
...
void setup() {
    Serial.begin(115200);
    debugPrint(2, "Starting Dynamic Electricity Ticker v6.2.4 (15-MINUTE DETAIL MODE) - DST-SAFE");
...
void loop() {
    ...
    if (timeinfo != nullptr) {
        ...
        // Calculate the current 15-minute block of the day (0-95)
        unsigned long current15MinBlock = (currentHour * 4) + (currentMinute / 15);

        // STATE-BASED REFRESH: 
        // We refresh if the Hour has changed OR the 15-minute block has changed.
        // This is safer than checking for "minute == 0" because it catches 
        // the change even if the ESP32 was busy exactly at the top of the hour.
        if (currentHour != (int)lastHourlyRefresh || current15MinBlock != last15MinRefresh) {
            debugPrint(2, "Time transition detected (H:" + String(currentHour) + " B:" + String(current15MinBlock) + "). Refreshing display...");
