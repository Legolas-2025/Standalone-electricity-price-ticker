/* 
  ESP32_standalone_electricity_ticker_6_1_nvs_daily_fetch.ino
  -----------------------------------------------------

  VERSION 6.1 CHANGES (2026-01-30):
  ---------------------------------
  GOALS:
  - Fix after-midnight automatic API fetch logic.
  - Distinguish between "HTTP/JSON success" and "data for today's market day".
  - Keep retrying after midnight until data for the new day appears.
  - Preserve working behavior for:
      * boot-time fetch
      * NVS load
      * manual long-press refresh

  MAIN BEHAVIOR CHANGES:
  1. "TODAY" DATA DETECTION:
     - Previously: only the first unix_seconds entry was checked against current local day.
     - Now:
       * Determine the "market day" of the dataset based on the LAST unix_seconds entry
         (assuming the data covers one full calendar/market day).
       * Compare that "data day" to the current local day.
       * If they differ, data is considered NOT for today → rejected logically.

  2. LOGICAL FAILURE VS HTTP/JSON FAILURE:
     - New global flag: lastProcessJsonAcceptedToday (bool).
     - Set to true only when processJsonData() accepts the dataset as "today".
     - handleDataFetching() now treats a fetch as SUCCESS only if:
         a) HTTP + JSON succeeded, AND
         b) lastProcessJsonAcceptedToday == true.
     - If (b) is false (dataset is for yesterday or wrong day):
         -> treated as failure for scheduling (keeps midnight retry logic running).

  3. MIDNIGHT RETRIES:
     - At local midnight rollover:
         -> midnightPhaseActive = true, midnightRetryCount = 0.
         -> nextScheduledFetchTime set to "now" (immediate attempt).
         -> UI forced into NO_DATA_OFFSET, LEDs off.
     - If data for the new day is not yet on the API:
         -> fetch returns HTTP 200 + JSON OK but dataset rejected (wrong day).
         -> handleDataFetching() treats this as failure (NOT success).
         -> scheduleAfterMidnightFailure() is called:
              a) Retry every 10 minutes up to 5 attempts.
              b) Then retry at each top of the hour.
     - As soon as the API provides a dataset whose LAST interval matches
       today's local day:
         -> processJsonData() accepts it.
         -> lastProcessJsonAcceptedToday = true.
         -> midnightPhaseActive is cleared.
         -> isTodayDataAvailable = true, UI leaves NO_DATA_OFFSET.

  4. NVS / MANUAL BEHAVIOR:
     - NVS load still uses the same "today" logic: only today's dataset is
       considered valid for boot reuse.
     - Manual long-press refresh now also respects the improved "today" logic:
       * will keep trying until the day's data is actually available.
*/

// ========================================================================
// Dynamic Electricity Ticker for XIAO ESP32C3 - VERSION 6.1
...
void displaySecondaryList() {
    ...
    snprintf(lines[18], sizeof(lines[18]), "price ticker v6.1 ");
...
void setup() {
    Serial.begin(115200);
    debugPrint(2, "Starting Dynamic Electricity Ticker v6.1 (15-MINUTE DETAIL MODE) - DST fixed");
