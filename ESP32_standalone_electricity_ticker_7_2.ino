/*
  ESP32_standalone_electricity_ticker_7_2.ino
  -----------------------------------------------------

  VERSION 7.2 CHANGES (2026-08-04):
  ----------------------------------
  Bug-fix release: primary-screen scroll, double-click and end-of-day logic
  (the three glitches reported on 2026-04-25 for the v7.1 firmware). No
  fee/VAT math, NVS layout, API or scheduling behaviour was changed.
...
// Set to true the first time we observe the button being released after
// boot. Until that happens the long-press detector refuses to trip, which
// prevents the ESP32-C3 floating-pin / power-rail noise (where the pin
// reads as pressed for several seconds after reset) from being treated
// as a 3-second hold.
...
void displayPriceRow(int row, int totalHourOffset, bool isCurrentHourRow) {
    ...
    if (getLocalTime(&timeinfo)) {
        ...
        if (!isCurrentHourRow && !showTomorrow) {
            // Hide any hour of TODAY that is already in the past.
            // The previous implementation only hid past hours while
            // currentHour < 22, which caused 22:00 / 23:00 to leak past
            // hours back onto the screen and produced the "scroll does
            // nothing" / blank-row behaviour the user reported.
...
void displayPrimaryList() {
    ...
    // NOTE: previous firmware forced displayStartHourOffset = 21 + timeOffsetHours
    // whenever currentHour >= 21. That override made scrolling visually do
    // nothing after 21:00 (e.g. at 22:00 the start stayed pinned at 22 no
    // matter how many times the user clicked). The blanking logic in
    // displayPriceRow() already hides past hours of today, so the override
...
void displaySecondaryList() {
    ...
    snprintf(lines[18], sizeof(lines[18]), "price ticker v7.2  ");
...
void advanceDisplayOffset() {
    ...
    // Doing it here also makes the secondary list scroll reset the timer,
    // which the previous implementation missed.
    ...
    if (displayState != NO_DATA_OFFSET) {
        ...
        // NOTE: previous firmware bumped allowedAhead up to 2 whenever it
        // was less than 2 in the 21:00-23:00 window with no tomorrow data.
        // That let users scroll into "24:00 / 25:00" hours, which then
        // wrapped displayStartHourOffset back to 0 and produced a screen
        // full of blank past-hour rows at 22:15 / 23:15. With the past-hour
        // blanking fix in displayPriceRow() the cap below is naturally
        // enough: at 22:00 you can step forward once (to 23:00), at 23:00
        // you can't step forward at all, and a wrap is never reached.
...
void handleButton() {
    // v7.2: v7.1 button logic restored verbatim. The only changes are
    // one extra line (buttonEverReleased = true) inside the existing
    // "reading went LOW" block, and one extra condition
    // (&& buttonEverReleased) on the long-press detector. Everything
    // else is byte-for-byte identical to v7.1, which is what worked
    // reliably with the TTP223.

    // NOTE on the reading convention used in this firmware:
    //   pinMode(buttonPin, INPUT_PULLUP);
    //   reading = !digitalRead(buttonPin);
    // So:
    //   reading == HIGH  <-> button is PRESSED
    //   reading == LOW   <-> button is RELEASED
    ...
    if (reading != lastButtonState) {
        ...
        if (reading == LOW) {
            ...
            // From this point on the long-press detector is allowed to
            // trip. Until this happens the detector refuses to fire,
            // which kills the false "Long press detected!" right after
            // a reset (where the pin is floating HIGH for the first
            // couple of seconds and buttonPressStartTime is still 0).
    ...
    if ((millis() - lastDebounceTime) > debounceDelay) {
        ...
        // `buttonEverReleased` guard. See comment above.
        if (buttonState == LOW && !longPressDetected && buttonEverReleased) {
...
void setup() {
    Serial.begin(115200);
    debugPrint(2, "Starting Dynamic Electricity Ticker v7.2 (Button Robustness) - DST-SAFE");
