/*
  ESP32_standalone_electricity_ticker_6_2_0.ino
  -----------------------------------------------------
...
*/
 ...
 // ========================================================================
 // Dynamic Electricity Ticker for XIAO ESP32C3 - VERSION 6.2.0
...
int findCurrentPriceIndex(const JsonArray& unixSeconds) {
    ...
    // Search for the first unixSeconds entry >= now
    for (size_t i = 0; i < unixSeconds.size(); i++) {
        unsigned long unixTime = unixSeconds[i].as<unsigned long>();
        if (!isValidUnixTime(unixTime)) continue;
    ...
    // If now is past all entries, return last index
    return (int)unixSeconds.size() - 1;
}
...
void displaySecondaryList() {
    ...
    snprintf(lines[18], sizeof(lines[18]), "price ticker v6.2.0");
...
void setup() {
    Serial.begin(115200);
    debugPrint(2, "Starting Dynamic Electricity Ticker v6.2.0 (15-MINUTE DETAIL MODE) - DST-SAFE");
